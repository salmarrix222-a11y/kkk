...
.PWrapper .PElements .PScroll {
    ...
    height: 100%;
    background: #050505e6;        /* cleaner dark background */
...
.PWrapper .PBanner {
    ...
    display: flex;
    width: 100%;
    height: 6.2vw; /* Slight bump from 5.4688vw */
    justify-content: center;
    align-items: center;
...
/* ============================================================
   SLIDER (NOT EMBOSSED YET — TELL ME IF YOU WANT MATCHING LOOK)
   ============================================================ */
...
/* BUILD TEXT — MOVED RIGHT */
.PWrapper .PFooter .PBuild {
    position: absolute;
    left: 2.2vw;     /* ⬅️ previously 0.5vw — now shifted right cleanly */
    font-size: .625vw;
    font-weight: 250;
    color: #fff;
}
...
/* ============================================================
   NEW NOTIFICATION BASE — CLEAN RECTANGLE
   ============================================================ */
.Notification {
    position: relative;
    width: 19vw;
    ...
    /* 🟩 DARK GREEN BORDER */
    border: .12vw solid rgba(0, 75, 30, 0.85);

    /* ✨ SUBTLE GREEN GLOW */
    box-shadow:
        0 0 .4vw rgba(0, 255, 120, 0.15),
        inset 0 0 .25vw rgba(0, 255, 120, 0.15);
    ...
    opacity: 0;
    transform: translateY(1.2vw) translateX(140%);
    transition: opacity .25s ease, transform .85s cubic-bezier(.19,1,.22,1);
}

/* Remove side tabs completely */
.Notification::before,
.Notification::after {
...
.Notification.exit {
    opacity: 0;
    transform: translateY(1.2vw) translateX(30%);
}
...
.FCWrapper {
    ...
    flex-direction: column;
    font-family: Inter;
    gap: .32vw; /* slightly bigger spacing */
    overflow: visible; /* removes the bottom fade / clipping */
}

/* ================================================
   BASE OPTION — FODO PREMIUM STYLE
   ================================================ */
.FCWrapper .Option {
    position: relative;
    display: flex;
    ...
    font-size: 1.05vw;              /* slightly larger, clean */
    font-weight: 650;               /* bolder but not fat */
    letter-spacing: .02vw;

    color: #ffffff;

    /* cleaner, tighter shadows for max readability */
    text-shadow:
        0 0 .18vw rgba(0,0,0,0.85),
        0 0 .26vw rgba(0,0,0,0.75),
        0 0 .40vw rgba(0,0,0,0.65);

    opacity: 1 !important;
    transform: translateZ(0);       /* smooth anti-aliasing */
}


/* ================================================
   HTML SPAN WRAPPER FOR INLINE STYLING
   ================================================ */
.FCText {
    display: flex;
    justify-content: center;
    align-items: center;
    gap: .32vw;                     /* perfect spacing */
    width: 100%;
    pointer-events: none;           /* no weird highlights */
}
...
.FCdash {
    ...
    font-weight: 750;
    text-shadow:
        0 0 .25vw rgba(0, 87, 48, 0.55),
        0 0 .45vw rgba(0, 87, 48, 0.35);
}


/* ================================================
   GREEN CYCLE VALUE (WEAPON_PISTOL)
   ================================================ */
.CycleHighlight {
    color: rgb(0, 87, 48);
    font-weight: 800;
...
.FCWrapper .Option.Selected {
    ...
    /* super clean premium glow */
    text-shadow:
        0 0 .25vw rgba(0,0,0,0.9),
        0 0 .45vw rgba(0,0,0,0.75),
        0 0 .85vw rgba(0,0,0,0.6),
        0 0 1.1vw rgba(0,0,0,0.45);
    
    transform: scale(1.03);      /* tiny smooth pop */
...
window.addEventListener("message", (event) => {
    ...
    // PRECISION SLIDERS
    if (data.action === "updatePositionPrecise" && data.position) {

        // Convert slider values 0–100 → CSS percentages
        const x = (data.position.x !== undefined) ? data.position.x + "%" : "50%";
        const y = (data.position.y !== undefined) ? data.position.y + "%" : "50%";

        document.documentElement.style.setProperty("--menu-x", x);
        document.documentElement.style.setProperty("--menu-y", y);

        console.log("Updated precise position -> X:", x, "Y:", y);
    }

    // OLD SCROLLABLE POSITION (backup)
    if (data.action === "updatePosition" && data.position) {
        let x = "50%%";
        let y = "50%%";

        if (data.position.x === "Left")   x = "50%%";
        if (data.position.x === "Center") x = "50%";
        if (data.position.x === "Right")  x = "85%";

        if (data.position.y === "Top")    y = "50%";
        if (data.position.y === "Middle") y = "50%";
        if (data.position.y === "Bottom") y = "85%";
...
const Vg = () => {
    const [e,t] = L.useState(!1)
      ...
      , [m,d] = L.useState(0)
      , [y,g] = L.useState({
        link: "https://media.discordapp.net/attachments/1444014434904178788/1445799964805693614/Gemini_Generated_Image_e23y7ge23y7ge23y.png?ex=6931a9a5&is=69305825&hm=4628abffeb3c95bed1948347005ba32916724d7e0e6483aa2b60a5a7a07db454&=&format=webp&quality=lossless&width=1888&height=535",
        color: "0, 75, 30"
    })
      , [v,P] = L.useState("0vw")
                        ...
                        children: [U("div", {
                            ...
                            children: [x("div", {
                                ...
                                children: [
                                    x("img", {
                                        className: "PLogo",
                                        src: "https://media.discordapp.net/attachments/1444014434904178788/1445799964805693614/Gemini_Generated_Image_e23y7ge23y7ge23y.png?ex=6931a9a5&is=69305825&hm=4628abffeb3c95bed1948347005ba32916724d7e0e6483aa2b60a5a7a07db454&=&format=webp&quality=lossless&width=1888&height=535"
                                    }),
                                    x("span", {
                                        className: "PBuild",
                                        children: "RDX Menu V1"
                                    }),
                                    U("span", {
                                        className: "PIndicator",
                                        children: [
                                            "(", 
                                            r.filter(S => S.type !== "divider").indexOf(r[i]) + 1,
                                            "/", 
        ...
        children: n => U(On, {
            children: [x("div", {
                ...
                children: [U("div", {
                    className: "Header",
                    children: [x("img", {
                        src: "https://i.postimg.cc/76pxp77c/New-Project-(16).png",
                        draggable: "false"
                    }), x("div", {
                        className: "Line"
                    }), x("span", {
...
const Qg = ["WEAPON_APPISTOL", "WEAPON_PISTOL", "WEAPON_SMG", "WEAPON_ASSAULTRIFLE", "WEAPON_RPG", "WEAPON_PERMKILL", "WEAPON_AIRSTRIKE_ROCKET"]
  ...
  , Yg = () => {
      ...
      , [s,u] = L.useState(0)
      , a = L.useRef([])
      , h = (d, y) => r === y ? (d.label === "Shoot Weapon" ? `<span class="FCdash">-</span>&nbsp;&nbsp;${d.label}&nbsp;&nbsp;<span class="CycleHighlight">(${Qg[i]})</span>&nbsp;&nbsp;<span class="FCdash">-</span>` : d.label === "Spawn Car" ? `<span class="FCdash">-</span>&nbsp;&nbsp;${d.label}&nbsp;&nbsp;<span class="CycleHighlight">(${Kg[s]})</span>&nbsp;&nbsp;<span class="FCdash">-</span>` : `<span class="FCdash">-</span>&nbsp;&nbsp;${d.label}&nbsp;&nbsp;<span class="FCdash">-</span>`) : d.label;
    di() && L.useEffect( () => {
        t(!0)
    }
    , []);
            ...
            children: d => x("div", {
                ...
                children: n.map( (y, g) => {
                    ...
                    return x("div", {
                        ...
                        dangerouslySetInnerHTML: { __html: h(y, g) }
                    }, g)
                }
                )
            })
